# -*- coding: utf-8 -*-
"""
Created on Sun Oct 11 23:48:56 2020
...
def connectToRobot(pn):
    global ser
    try:
        ser = serial.Serial(pn, 9600)
...
while True:
    ...
    imgMask, imgColorFilter = colorFilter(img,trackBarPos)
 
    imgCropped = imgMask[cv[1]:cv[2]+cv[1],cv[0]:cv[0]+cv[3]]
    imgResult = imgResult[cv[1]:cv[2] + cv[1], cv[0]:cv[0] + cv[3]]
    ...
    cv2.rectangle(img, (cv[0], cv[1]), (cv[0]+cv[3], cv[2]+cv[1]), (255, 0, 0), 2)
